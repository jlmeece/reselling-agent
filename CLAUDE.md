...
- Sheet grid guard: `researcher.py` calls `ensure_grid_columns` at start so the tab always has ≥ `required_grid_columns()` (53, from col_map.yaml). Before this (2026-09-18 → 09-23) MPT cols AX–BA exceeded the 49-col grid and research exited 1. `sheet_formatter` no longer shrinks the grid below that. MPT columns AX–BA still have no header labels
